...
すでに運用中のスクリプトを更新した場合は、`primeFingerprints()` を一度実行しておくとよい。繰り返し予定の巻き添え通知の抑止に使う指紋をまとめて記録するので、更新後の最初の変更から抑止が効く。

`primeFingerprints()` は `syncToken` を進めず、まだ通知していない差分に含まれる予定は記録しない（その変更は次の定期実行が通知する）。定期実行と同じロックを取るので、走っている間に叩くとエラーで中止する。カレンダーAPIから差分を取れなかった場合も、指紋を書かずにエラーになる（通知を消すより止まるほうを選ぶ）。
