...
`SYNC_TOKEN:<カレンダーID>` と `SEEN_EVENTS:<カレンダーID>`（8KBを超えた分は `:1` のような続きのシャード）はスクリプトが自動で保存するため手動設定は不要。
...
繰り返し予定の巻き添え通知を抑止するための指紋（下敷き）は、**定期実行が最初の1回で自動的に作る**ので手作業は要らない。`FINGERPRINTS_PRIMED_AT:<カレンダーID>` がその目印で、値には下敷きの形式番号（`v2|` など）が付く。指紋の取り方が変わる更新を入れたときは、定期実行が下敷きを自動で作り直す。
