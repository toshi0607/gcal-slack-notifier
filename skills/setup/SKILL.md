...
`SYNC_TOKEN:...` と `SEEN_EVENTS:...` は自動生成されるため設定不要。
...
すでに運用中のスクリプトを更新した場合は `initialize()` を実行し直す必要はない。繰り返し予定の巻き添え通知を抑止するための指紋は、次の定期実行が自動で記録する。トリガーを待たずに埋めたいときだけ `primeFingerprints()` を手動実行する（`syncToken` を作り直さず、まだ通知していない差分に含まれる予定は記録しないので、通知を取りこぼさない）。
