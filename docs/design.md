...
| `resetBaseline_()` | フル同期して `syncToken` と `LAST_SYNC_AT` を書く。初期化・新規カレンダー・`syncToken` 失効の共通処理 |
...
| `isUnchangedResend_(ev, lastSyncAt)` | 今回変更されていないのに差分へ巻き添えで乗ったイベントかを判定。通知対象から落とすために使う |
| `readLastSyncAt_(props, calendarId)` | `LAST_SYNC_AT:<カレンダーID>` を `Date` で読む。未記録・不正なら `null` |
...
繰り返し予定を1回分だけ削除しても、差分にはその回だけが乗るとは限らない。シリーズ本体や、以前に個別変更・削除した**未来の**回まで一緒に返ってくる（2026-08-14 に実測: 9/2 の1回を削除しただけで、シリーズ本体2件・別の回の削除2件を巻き添えに計5件のSlack通知が飛んだ）。「過ぎた回の除外」は日付が今日より前の回しか落とさないので、未来の回の巻き添えはそのまま通知になってしまう。

巻き添え分は再送されただけで中身は変わっていない。つまり `updated`（そのイベント自身が最後に更新された時刻）が古いままになる。そこで**前回の差分取得を開始した時刻**を `LAST_SYNC_AT:<カレンダーID>` に記録しておき、`isUnchangedResend_()` で `updated` がそれより前のイベントを落とす。今回実際に変更されたイベントだけが新しい `updated` を持つので、「変更した予定そのもの」だけが残る。

| 判断 | 理由 |
|---|---|
| 記録するのは差分取得を**始める前**の時刻 | 取得中に入った変更は次回の差分に乗る。終わってから測ると、その分を「前回より前の更新」と誤判定して落としてしまう |
| 比較に 60秒 の許容誤差（`UPDATED_SKEW_MS`）を置く | `updated` はカレンダー側の時刻、基準はスクリプト側で測った時刻。時計のずれと差分APIへの反映遅れを吸収する。通知が重複するより消えるほうが困るので、迷ったら通す側に倒す |
| 誤差を大きくしすぎない | 広げるほど「前回の実行で通知済みの変更」が再送で二重に通知される窓が広がる。繰り返し予定の回を続けて何件も消すような操作で効いてくる |
| `updated` が無い・`LAST_SYNC_AT` が未記録／不正なら落とさない | 判定材料がないときに黙って通知を消さない。この機能より前から動いている環境は、初回の実行だけ従来どおり全件通知して基準を作る |

`LAST_SYNC_AT` は `syncToken` と同じ場面（`initialize()`・新規カレンダーの基準点作成・`syncToken` 失効時の再初期化）で書き直す。両者は対で意味を持つため、`resetBaseline_()` にまとめて片方だけ古くなるのを防ぐ。

なお、この除外はシリーズ本体そのものへの変更には効かない。シリーズのタイトルを変えれば本体の `updated` が進むので、従来どおり「✏️ 予定が更新されました」が1件飛ぶ。
...
| `LAST_SYNC_AT:<カレンダーID>` | 前回の差分取得を開始した時刻（ISO 8601）。巻き添えの再送を落とすのに使う | `initialize()` と定期実行が自動保存 |
