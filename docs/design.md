...
| `isStaleCancellation_(ev, lastRunAt)` | 前回の実行より前に消された削除かを判定。通知対象から落とすために使う |
| `readLastRunAt_(store, calendarId)` | `LAST_RUN_AT:<カレンダーID>` を `Date` で読む。未記録・不正なら `null` |
...
指紋の下敷きに載っていない回が再送されると、初見なので通知される。とくに**未来のキャンセル済みの回**（前に消した回）がこれに当たる（2026-08-21 に実測: ごはんの回を1つ消したところ、以前に消した 9/17・10/1 の分まで「削除されました」と通知された）。下敷きはフル同期で作るが、キャンセル済みの回は `start` を持たないことがあり、`timeMin` で絞ったフル同期に現れるとは限らない。

そこで、指紋とは別の観点で `isStaleCancellation_()` を置く。**削除に限れば `updated` は「いつ消したか」を表す**ので、前回の実行より前に消されていた回は落とす。すでに知らせたか、このスクリプトを入れる前の削除のどちらかで、いま知らせても意味がない。

| 判断 | 理由 |
|---|---|
| 判定を削除（`status: cancelled`）に限る | `updated` は main event data の最終更新時刻で、リマインダーだけの変更では進まない。更新イベントの判定には使えないが、削除は必ず `updated` が動くのでこの用途でだけ意味を持つ |
| 基準は前回の**実行開始**時刻（`LAST_RUN_AT:<カレンダーID>`） | 取得を始める前に測るので、実行中に入った削除を誤って落とさない |
| 60秒（`CANCELLED_SKEW_MS`）の余裕を置く | `updated` はカレンダー側、基準はスクリプト側の時刻。差分APIへの反映遅れも含めて緩めに見る。迷ったら通知する側に倒す |
| 未記録・`updated` が読めないときは落とさない | 判定材料がないときに黙って通知を消さない |
...
| `recurrence` の **EXDATE / RDATE** も除外する | 回を1つ削除・移動すると本体側に積まれる（2026-08-21 に実測: 回を消しただけで本体の更新通知が出続けていた）。「その回をどうしたか」は回そのものの通知が伝えているので、本体から言い直す必要がない。RRULE は残すため、繰り返しルール自体の変更は通知される |
...
| `LAST_RUN_AT:<カレンダーID>` | 前回の差分取得を開始した時刻。済んだ削除を落とすのに使う | 定期実行が自動保存 |
