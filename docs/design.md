...
| `eventFingerprint_(ev)` | イベントの中身の指紋を返す。削除済みは墓標（IDだけの指紋）になる |
| `tombstoneFingerprint_(ev)` | 削除済みイベントの墓標。「このIDが削除済みであること」だけを表す |
| `semanticFingerprint_(ev)` | 管理情報（`BOOKKEEPING_FIELDS`）を除いた指紋を返す。削除済みは墓標になる |
| `canonicalize_(value, ignoredFields)` | 指紋を取るための正規化。キーをソートし、無視するフィールドを落とす |
| `digest_(text)` | MD5の先頭8バイトを16進で返す |
| `readSeenEvents_()` / `writeSeenEvents_()` / `toFingerprintMaps_()` | `SEEN_EVENTS:<カレンダーID>`（と続きのシャード）の読み書きと引き当て。書き込み時に過去回を除き、8KBごとにシャードへ分ける |
| `isPastOccurrenceId_(id, cutoff)` / `occurrencePruneCutoff_()` | 保存済みエントリの掃除。IDに埋まる回の日付が過去かを判定する |
...
そこで、差分で見たイベントの**指紋**（中身のハッシュ）を `SEEN_EVENTS:<カレンダーID>` に残しておき、`isUnchangedResend_()` で「前回見たときと指紋が同じ」イベントを落とす。

指紋の取り方はイベントの状態で分ける。

- **confirmed（現役）**: 中身全体のハッシュ。リマインダーだけの変更も `reminders` の差として出る
- **cancelled（削除済み）**: 中身を見ず、IDだけの**墓標**（`digest('cancelled:' + id)`）。差分に乗る削除済みのペイロードは、親シリーズや兄弟の回を操作するたびに `updated` などの管理情報が動くため、中身の指紋では二度と一致しない（2026-08-22 実測: 06:11 に回を1件消したら削除済みの回が巻き添え通知され、**5分後**の 06:16 に別の回を消したら**同じ削除済みの回がまた通知された**。指紋は5分前に確実に記録済みだった。同じ2実行でシリーズ本体は `updated`・`sequence` を除いた指紋の一致で黙っていた＝動いたのは管理情報だけ）。削除済みに意味のある変化は「復活」だけで、そのとき status は confirmed に戻って中身の指紋になり、墓標と一致しなくなるので通知される
...
| 削除済みの指紋に中身を使わない | 上記の実測のとおり、削除済みの中身は操作のたびに動く。中身の指紋で比べるかぎり、削除済みの回の巻き添え通知は**何度でも再発する**（v2 で墓標に変更） |
...
| 指紋は保存量に上限を設け、古いものから捨てる | 溢れた分は「初見扱い」に戻るだけで、通知が消える方向には倒れない（保存の詳細は「指紋の保存量とシャード」） |
| **過ぎた回は記録しない** | `isPastOccurrence_()` は保存の有無に関係なく毎回落とすので、記録しても抑止には寄与せず容量だけを食う（2026-08-25 実測: 保存枠119件のうち約90件が2021〜2025年の過去回で埋まり、現役の指紋を押し出していた）。保存済みのエントリも、IDに埋まる回の日付（`_YYYYMMDD(THHMMSSZ)?` 形式。UTCなので2日の猶予付き）が過去になったものから掃除する |
...
Script Properties は1プロパティ 9KB まで。1プロパティ（約110エントリ）では繰り返し予定の多いカレンダーの現役の指紋を持ちきれない（2026-08-25 実測: 8KB の枠が2シリーズ分の回だけで埋まって他の全シリーズの指紋が押し出され、どのシリーズを触っても巻き添え通知が再発し続けた。押し出しの多くは記録する意味のない過去回だった）。

そこで指紋は新しい順のまま **8KB ごとに最大4つのシャード**（`SEEN_EVENTS:<カレンダーID>`、`:1`、`:2`、`:3`）へ分けて保存する（計約32KB・500エントリ。ストア全体の上限は500KB）。

- シャード0は従来のキーそのもので後方互換。読み取りは `getProperties()` 1回にまとめたままなので、シャードが増えても読み取りの呼び出し回数は増えない
- 縮んだら余ったシャードを**先に**消してから、新しい内容を書く。逆順だと途中で失敗したときに前世代のシャードが残り、捨てたはずの指紋が生き返る。先に消す順なら、途中で失敗しても消えた分が初見扱いに戻る（＝1回多く通知される）だけで、通知が消える側には倒れない
- 読めない・欠けたシャードは飛ばす（その分を落とさないだけ）。同じ番号のシャードの書き込みが途中で失敗して世代が混ざっても、今回の差分を含むシャード0から先に書くこと＋同じIDの新しい順の先勝ちで、新しい指紋が使われる
- それでも溢れた分は古い順に捨てる（初見扱いに戻るだけ＝安全側）
...
そこで、フル同期のついでに指紋を記録する。記録済みかどうかは `FINGERPRINTS_PRIMED_AT:<カレンダーID>` で持つ。マーカーの値は `v2|<ISO日時>` のように**下敷きの形式番号**を含み、指紋の取り方を変えたら番号を上げる。古い形式のマーカーしか無いカレンダーは下敷きを作り直す（古い取り方の指紋は新しい指紋と一致せず、抑止に使えないため）。

- **定期実行が、下敷きの無い・古い形式のカレンダーを見つけたら1回だけ自分で作り直す。** マーカーが無ければ（または形式が古ければ）差分の取得より先にフル同期し、その実行で通知する差分を除いて記録する。デプロイしただけで次のトリガーが下敷きを作るので、手作業は要らない
...
| 権限不足・値のサイズ超過・1日あたりの呼び出し上限（`PERMANENT_STORAGE_ERROR`）だけは再試行せず即座に諦める | 待っても直らない。設定不備を再試行のログで埋もれさせない。呼び出し上限は `for one day`（日次）に限る。`in a short time` の短時間レート制限は待てば直るので再試行する側 |
...
| `SEEN_EVENTS:<カレンダーID>`（続きは `:1` `:2` `:3`） | 直近に見たイベントの指紋 `[[イベントID, 中身の指紋, 管理情報を除いた指紋?], ...]`（3つめはシリーズ本体のみ。削除済みの指紋は墓標）。8KBごとに最大4シャード。巻き添えの再送を落とすのに使うキャッシュ | 定期実行が自動保存 |
| `FINGERPRINTS_PRIMED_AT:<カレンダーID>` | 指紋の下敷きの形式と作った時刻（`v2\|<ISO日時>`）。形式が古ければ定期実行が作り直す | 定期実行が自動保存 |
