...
| 読み書きを最大 `STORAGE_ATTEMPTS`（4回）まで指数バックオフ（500ms → 1s → 2s）で再試行する | この障害は数百ミリ秒から数秒で復旧することが多い。読み取り・書き込み・削除はいずれも冪等なので、原因を問わず再試行してよい |
...
- スクリプト プロパティの一時障害（`Error code INTERNAL`）は最大4回まで指数バックオフで再試行する。使い切ったら例外にして失敗通知メールに載せる（詳細は「スクリプト プロパティの一時障害」）
